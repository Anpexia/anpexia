...
import { demoJFService } from './demo-jf.service';
...
router.post('/chat', async (req: Request, res: Response) => {
  try {
    ...
    const result = await demoJFService.chat(sessionId, message);
    return res.json(result);
  } catch (err: any) {
    console.error('[DEMO-JF] Error:', err.message);
...
export const demoJFRouter = router;
