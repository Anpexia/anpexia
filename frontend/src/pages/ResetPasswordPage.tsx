...
export function ResetPasswordPage() {
  ...
  if (loadingValidate) return <div className="min-h-screen flex items-center justify-center text-slate-500">Validando link...</div>;
    ...
    return (
      <div className="min-h-screen bg-[#EFF6FF] flex items-center justify-center px-4">
        <div className="bg-white rounded-xl shadow-sm border border-red-200 p-8 max-w-sm text-center">
          <h1 className="text-xl font-bold text-red-700 mb-2">Link invalido</h1>
          <p className="text-slate-600 text-sm">{validationError}</p>
          <button onClick={() => navigate('/recuperar-senha')} className="mt-4 btn-pill btn-primary">Solicitar novo link</button>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#EFF6FF] flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-[#BFDBFE] p-6 space-y-4">
          ...
          <p className="text-sm text-slate-600">Crie uma nova senha para {userInfo?.email}.</p>
